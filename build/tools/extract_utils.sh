...
function write_header() {
    ...
    printf "# Copyright (C) $YEAR The halogenOS Project\n" >> $1
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$CUSTOM_ROOT"/vendor/aosp/build/tools/smali/baksmali.jar
        export SMALIJAR="$CUSTOM_ROOT"/vendor/aosp/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$CUSTOM_ROOT"/vendor/aosp/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$CUSTOM_ROOT"/vendor/aosp/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$CUSTOM_ROOT"/vendor/aosp/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
