...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/XOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.product.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop ro.product.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.product.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
